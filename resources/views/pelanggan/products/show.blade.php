...
<div class="container mt-5 py-5">
    <div class="row shadow-sm bg-white rounded mb-0">
        {{-- Gambar Produk --}}
        <div class="col-md-6">
            <div class="img-product p-3 text-center mx-auto mt-3 mb-3" style="min-height: 400px; width: 100%; max-width: 900px;">
                ...
                <form action="{{ route('pelanggan.products.preorder', $product->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select name="material_id" id="material" class="form-control @error('material_id') is-invalid @enderror" required>
                                ...
                                @foreach ($product->materials as $material)
                                    <option value="{{ $material->id }}" data-price="{{ $material->pivot->additional_price }}" {{ old('material_id') == $material->id ? 'selected' : '' }}>
                                        {{ $material->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="size_id" id="size" class="form-control @error('size_id') is-invalid @enderror" required>
                                ...
                                @foreach ($product->sizes as $size)
                                    <option value="{{ $size->id }}" data-price="{{ $size->pivot->additional_price }}" {{ old('size_id') == $size->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <div class="form-check">
                            <input class="form-check-input @error('design_input_type') is-invalid @enderror" type="radio" name="design_input_type" id="uploadOption" value="upload" {{ old('design_input_type', 'upload') === 'upload' ? 'checked' : '' }} onchange="toggleDesignInput()">
                            <label class="form-check-label" for="uploadOption">Upload File Desain</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input @error('design_input_type') is-invalid @enderror" type="radio" name="design_input_type" id="linkOption" value="link" {{ old('design_input_type') === 'link' ? 'checked' : '' }} onchange="toggleDesignInput()">
                    ...
                    <div class="form-group d-none" id="linkInput">
                        ...
                        <input type="url" name="design_link" id="design_link" class="form-control @error('design_link') is-invalid @enderror" placeholder="https://drive.google.com/..." pattern="https?://.+">
                        @error('design_link')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
...
{{-- Deskripsi Produk --}}
    <div class="py-5">
        <h5 class="sub-heading">Deskripsi Produk</h5>
